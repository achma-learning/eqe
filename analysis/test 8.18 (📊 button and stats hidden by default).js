...
// @version      8.18
// @description  ←→↑↓ nav • T/8 loadouts • Space/Enter check • H sidebar (works on course page too) • F fullscreen • P session timer • Shift+? help • Auto-advance HUD island • Course switcher (M) • Dashboard 1-9 nav • Pomo page-glow • Course page: hide images + compact layout • Hide stats panel (📊 toggle)
...
(() => {
    ...
    // ── prevent double-init (hot-reload / stale fallback timer) ─────────────
    ...
    const TRANSIENT_TEXTS = new Set(['command palette', 'command', 'search']);
    const getQuestionText = () => {
        const el = document.querySelector('h1, h2, [class*="question"]');
        if (!el) return null;
        const text = el.textContent.trim();
        if (text.length < 10) return null;
        if (TRANSIENT_TEXTS.has(text.toLowerCase())) return null;
        return text;
    };
...
<svg id="eqe-pomo-svg" viewBox="0 0 36 36"
  ...
  <text id="eqe-pomo-text" x="18" y="18"
    ...
    function injectInlineControls() {
        ...
        [btnLoadout, btnCourse, btnToggle, btnSidebar, btnShortcuts, btnPomo, btnGear]
