...
(() => {
    ...
    let config = {
        ...
        includeExamName:     GM_getValue('includeExamName',     true),   // NEW: add exam name to prompt
    };
    ...
    // Enhanced question text extraction (filter placeholders & use precise selectors)
    ...
    function copyFullPrompt() {
        const prompt = buildFullPrompt(config.includeCorrection, config.includeExamName);
        if (!prompt) {
...
<svg id="eqe-pomo-svg" viewBox="0 0 36 36"
  ...
  <text id="eqe-pomo-text" x="18" y="18"
    ...
    // ============================================================================
    // SETTINGS UI (added checkbox for "include exam name")
    // ============================================================================
    ...
    function createSettingsPanel() {
        ...
        panel.innerHTML = `
            ...
            <div style="margin-bottom:15px;padding-top:15px;border-top:1px solid ${c.borderTop};">
                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                    <input type="checkbox" id="eqe-include-examname" ${config.includeExamName?'checked':''} style="width:18px;height:18px;cursor:pointer;">
                    <span style="font-weight:600;color:${c.labelColor};">📝 Add exam name to the prompt</span>
                </label>
                <small style="color:${c.smallColor};margin-left:28px;display:block;margin-top:4px;">When enabled, the prompt will include the exam title (e.g., "Examen : normal 2026").</small>
            </div>
            <div style="margin-bottom:15px;padding-top:15px;border-top:1px solid ${c.borderTop};">
    ...
    function saveSettings() {
        ...
        const includeExamName     = document.getElementById('eqe-include-examname')?.checked ?? true;
        ...
        config.includeExamName     = includeExamName;
        ...
        GM_setValue('includeExamName',     includeExamName);
